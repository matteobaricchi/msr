...
#%%
# pp function - version created fro WES review


def calculatePmat_withUncertainty_pp(wfm,
                                 x,
                                 y,
                                 wd_array,
                                 ws_array,
    ...
    for nn in np.arange(n):
        ...
        #power_delta_wd_mat_temp = wfm(x,y,wd=(wd_array+delta_wd)%360,ws=ws_array,yaw=yaw_ordered+delta_wd,tilt=np.zeros_like(yaw),helix_amp=helix_amp_ordered).Power.values
        simres = wfm(x,y,wd=(wd_array+delta_wd)%360,ws=ws_array,yaw=yaw_ordered+delta_wd,tilt=np.zeros_like(yaw),helix_amp=helix_amp_ordered)
        ws_eff_delta_wd_mat_temp = simres.WS_eff_ilk

        # previous function (normal power extraction)
        #power_delta_wd_mat_temp = wfm.windTurbines.power(ws_eff_delta_wd_mat_temp*np.cos(np.deg2rad(yaw_ordered+delta_wd)))

        # previous function (trying to understand the pp coefficent used from PyWake in every case) -> not working
        #power_default_delta_wd_mat = wfm.windTurbines.power(ws_eff_delta_wd_mat_temp*np.cos(np.deg2rad(yaw_ordered+delta_wd)))
        #power_noYaw_delta_wd_mat = wfm.windTurbines.power(ws_eff_delta_wd_mat_temp)
        #f_default_yaw_mat = power_default_delta_wd_mat/power_noYaw_delta_wd_mat
        #f_new_yaw_mat = (np.cos(np.deg2rad(yaw_ordered+delta_wd)))**pp

        f_new_yaw_mat = (np.cos(np.deg2rad(yaw_ordered+delta_wd)))**pp
        f_default_yaw_mat = (np.cos(np.deg2rad(yaw_ordered+delta_wd)))**pp_PyWake
        power_delta_wd_mat_temp = np.minimum(simres.Power.values*f_new_yaw_mat/f_default_yaw_mat,p_rated)
        
        #print(f'f default: {np.min(f_default_yaw_mat)} - {np.mean(f_default_yaw_mat)} - {np.max(f_default_yaw_mat)}')
        #print(f'f pp188: {np.min(f_new_yaw_mat)} - {np.mean(f_new_yaw_mat)} - {np.max(f_new_yaw_mat)}')
        #print(f'Power multiplier: {np.min(f_new_yaw_mat/f_default_yaw_mat)} - {np.mean(f_new_yaw_mat/f_default_yaw_mat)} - {np.max(f_new_yaw_mat/f_default_yaw_mat)}')
        #pp_coeff = np.log(f_default_yaw_mat)/np.log(np.cos(np.deg2rad(yaw_ordered+delta_wd)))
        #print(f'pp coeff PyWake: {np.min(pp_coeff)} - {np.mean(pp_coeff)} - {np.max(pp_coeff)}')
...
def calculatePmat_withUncertainty_Bast(wfm,
                                 x,
                                 y,
                                 wd_array,
                                 ws_array,
    ...
    for nn in np.arange(n):
        ...
        power_delta_wd_mat_temp = wfm(x,y,wd=(wd_array+delta_wd)%360,ws=ws_array,yaw=yaw_ordered+delta_wd,tilt=np.zeros_like(yaw)).Power.values
        power_delta_wd_mat[:,:,:,nn] = np.take_along_axis(power_delta_wd_mat_temp,ind_array_post[na,:,na],axis=1)
